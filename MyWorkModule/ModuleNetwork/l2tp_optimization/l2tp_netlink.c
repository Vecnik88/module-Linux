			       ...
			       struct l2tp_tunnel *tunnel)
{
	...
	u64 rx_seq_discards = 0, rx_oos_packets = 0;
	...
	for_each_possible_cpu(i) {
		...
		unsigned int start;
		u64 t_bytes, t_packets, t_errors;
		u64 r_bytes, r_packets, r_errors, r_seq_discards, r_oos_packets;
		...
		do {
			...
			r_seq_discards = t_stats->rx_seq_discards;
			r_oos_packets = t_stats->rx_oos_packets;
		...
		rx_errors += r_errors;
		rx_seq_discards += r_seq_discards;
		rx_oos_packets += r_oos_packets;
	}

	if (nla_put_u64(skb, L2TP_ATTR_TX_PACKETS, tx_packets) ||
	    nla_put_u64(skb, L2TP_ATTR_TX_BYTES, tx_bytes) ||
	    nla_put_u64(skb, L2TP_ATTR_TX_ERRORS, tx_errors) ||
	    nla_put_u64(skb, L2TP_ATTR_RX_PACKETS, rx_packets) ||
	    nla_put_u64(skb, L2TP_ATTR_RX_BYTES, rx_bytes) ||
	    nla_put_u64(skb, L2TP_ATTR_RX_SEQ_DISCARDS, rx_seq_discards) ||
	    nla_put_u64(skb, L2TP_ATTR_RX_OOS_PACKETS, rx_oos_packets) ||
	    nla_put_u64(skb, L2TP_ATTR_RX_ERRORS, rx_errors))
		goto nla_put_failure;
				...
				struct l2tp_session *session)
{
	...
	int i;
	u64 rx_bytes = 0, tx_bytes = 0, rx_errors = 0;
	u64 tx_errors = 0, rx_packets = 0;
	u64 tx_packets = 0, rx_seq_discards = 0, rx_oos_packets = 0;
	...
	for_each_possible_cpu(i) {
		...
		do {
			...
			r_seq_discards = s_stats->rx_seq_discards;
			r_oos_packets = s_stats->rx_oos_packets;
