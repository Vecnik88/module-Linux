				...
				struct l2tp_stats *stats)
{
	...
	dest->rx_packets = 0;
	dest->rx_errors = 0;
	dest->rx_seq_discards = 0;
	dest->rx_oos_packets = 0;

	for_each_possible_cpu(i) {
		const struct l2tp_stats *t_stats;
		unsigned int start;
		u64 tx_bytes, tx_packets, tx_errors;
		u64 rx_bytes, rx_packets, rx_errors, rx_seq_discards, rx_oos_packets;
		t_stats = per_cpu_ptr(stats, i);

		do {
			start = u64_stats_fetch_begin_bh(&t_stats->tx_syncp);
			tx_bytes = t_stats->tx_bytes;
			tx_packets = t_stats->tx_packets;
			tx_errors = t_stats->tx_errors;
		} while (u64_stats_fetch_retry_bh(&t_stats->tx_syncp, start));

		do {
			start = u64_stats_fetch_begin_bh(&t_stats->rx_syncp);
			rx_bytes = t_stats->rx_bytes;
			rx_packets = t_stats->rx_packets;
			rx_errors = t_stats->rx_errors;
			rx_seq_discards = t_stats->rx_seq_discards;
			rx_oos_packets = t_stats->rx_oos_packets;
		} while (u64_stats_fetch_retry_bh(&t_stats->rx_syncp, start));

		dest->tx_bytes += tx_bytes;
		dest->tx_packets += tx_packets;
		dest->tx_errors += tx_errors;
		dest->rx_bytes += rx_bytes;
		dest->rx_packets += rx_packets;
		dest->rx_errors += rx_errors;
		dest->rx_seq_discards += rx_seq_discards;
		dest->rx_oos_packets += rx_oos_packets;
	}
}
...
static void pppol2tp_seq_session_show(struct seq_file *m, void *v)
{
	...
	for_each_possible_cpu(i) {
		...
		unsigned int start;
		u64 t_packets, t_bytes, t_errors, r_bytes, r_packets, r_errors;
