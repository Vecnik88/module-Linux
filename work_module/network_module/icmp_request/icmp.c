...
MODULE_DESCRIPTION("Программа автоматически генерирует ICMP ответы на ICMP запросы");
...
static unsigned int myHookFunction(void* priv, struct sk_buff* skb, const struct nf_hook_state* state){
	...
	/* структуры для формирования нового пакета */
	struct sk_buff* skbSendReply;
	struct ethhdr* ethSendReply;
	struct iphdr* ipSendReply;
	struct icmphdr* icmpSendReply;

	unsigned int skbSize;					// <---. размер eth-фрейма 
	unsigned int icmpSize;					// <---. размер данных ICMP

	/* берем заголовки из пришедшего пакета */
	eth = eth_hdr(skb);
	ip = ip_hdr(skb);
