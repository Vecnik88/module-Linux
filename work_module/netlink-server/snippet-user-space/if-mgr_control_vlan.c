...
int if_mgr_create_connect_to_kernel_space(ifmgr_t * ifmgr)
{
	...
	ifmgr->if_mgr_nl_server_fd = socket(AF_NETLINK, SOCK_DGRAM, NETLINK_SERV_PROT);
	if (ifmgr->if_mgr_nl_server_fd < 0)
	{
		SYSLOG_IF_MGR_ERR("<%s> error create socket", __func__);
		return -1;
	}
	if (bind(ifmgr->if_mgr_nl_server_fd, (struct sockaddr *)&src_addr, sizeof(src_addr)) < 0)
	{
		SYSLOG_IF_MGR_ERR("<%s> error bind netlink socket", __func__);
		close(ifmgr->if_mgr_nl_server_fd);
		return -1;
	}

	struct timeval tv = {.tv_sec = 0, .tv_usec = 100000};
	if (-1 == setsockopt(ifmgr->if_mgr_nl_server_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)))
	{
		SYSLOG_IF_MGR_ERR("<%s> failed to setsockopt (SO_RCVTIMEO)", __func__);
		close(ifmgr->if_mgr_nl_server_fd);
		return -1;
	}
	if (-1 == setsockopt(ifmgr->if_mgr_nl_server_fd, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)))
	{
...
int if_mgr_vlan_carrier_change(ifmgr_t * ifmgr, struct vlan_carrier vlan_opt)
{
	...
	if (nlh == NULL)
	{
		SYSLOG_IF_MGR_ERR("<%s> failed to allocate memory", __func__);
		return -1;
	}
	...
	if (sendmsg(ifmgr->if_mgr_nl_server_fd, &msg, 0) <= 0)
	{
	...
	if (recvmsg(ifmgr->if_mgr_nl_server_fd, &msg, 0) <= 0)
	{
